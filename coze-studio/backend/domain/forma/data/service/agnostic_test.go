...
func TestProductionDataDomainIsIndustryAgnostic(t *testing.T) {
	dataRoot := filepath.Clean("..")
	...
	err := filepath.WalkDir(dataRoot, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			if strings.Contains(normalized, "/fixture") || strings.Contains(normalized, "/testdata") {
