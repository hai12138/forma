...
沿用系统字体、白色点阵工作面、16px 圆角、轻量阴影及原语义色；不修改全局 Tokens 或 Shell。新增局部组件 VisualBusinessModelEditor，顶部编辑工具栏，左侧选择/平移/适配，右侧 250px 属性与治理面板，底部图例与缩放。小屏将属性区折到下方，全屏保留工具与属性区。

节点显示类型、AI/人工来源、名称与描述；双击聚焦名称。节点右侧圆点支持拖到目标节点建立关系，也可通过侧栏选择起点后点击目标。连线有标签、箭头和加宽点击区域。属性保存为独立语义提交，空白名称/关系标签不可提交。Delete 删除选中项，Ctrl/Cmd+Z 撤销，Ctrl/Cmd+Shift+Z 或 Ctrl/Cmd+Y 重做；输入框内快捷键不触发模型删除/撤销。

接口为 value: VisualModel、onChange(value, action)、onImpact()。渲染器无平台状态依赖；治理与本地持久化由 Store 适配。语义边使用 from/target 表示端点，source 表示来源；canvasGraph() 转成原 BusinessGraph 的 source/target，避免两个 source 概念冲突。详见 [模型及边界](06-visual-model-editor.md)。
...
## v1.1 Business Canvas 增量

所有业务图使用 `components/business-canvas.tsx` 与 `lib/business-canvas.ts`。语义颜色、组件接口、自动布局边界、状态联动和验收步骤详见 [Business Canvas 交接](05-business-canvas.md)。既有设计 Tokens 和全局主题未变；新增样式局部作用于画布。
