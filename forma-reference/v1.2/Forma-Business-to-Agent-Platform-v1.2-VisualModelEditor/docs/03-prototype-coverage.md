...
本轮完整支持节点拖拽、六类节点新增/删除、名称/描述/类型编辑、关系创建/标签编辑/删除、撤销重做、自动/手动布局、保存/恢复布局、Fit/Zoom/Fullscreen，以及来源记录与语义版本治理。

浏览器实测与领域测试结果见 [verification](04-verification.md)；完整对应用户 17 项需求的矩阵见 [编辑器交接](06-visual-model-editor.md)。AI 重新布局是明确标识的本地确定性算法模拟，没有调用真实 LLM。保存为本机 localStorage，不是跨设备数据库或多人协作。

体验步骤：/business → Fit View → 新增角色 → 编辑属性 → 从节点创建关系 → 选择目标 → 修改关系标签 → 拖动节点 → 保存布局 → 刷新 → 撤销/重做 → 查看变更影响 → 重新确认与评测。
...
16 个一级路由可以直接访问，子视图用页内 Tabs；当前不提供每个 Tab 的独立 URL。业务模型与应用重点展示一个园区场景；总览的其他应用与部分统计为展示数据。Application 节点仍只用于配置选择；Business Canvas 在 v1.2 支持手动编辑与语义快照。原生产版本对比仍为演示数据，不是通用服务端版本引擎。
...
/business 主图及变更影响、/analyst 候选模型摘要、/data 数据归属、/capabilities 依赖、/applications 协作均使用共享 BusinessCanvas。选中、平移、缩放、适配、全屏已有实现；导出仅占位。规则与审批、数据模式、Agent 选择和编排方式从原状态读取。详见 [逐页覆盖与边界](05-business-canvas.md)。
