# v1.2 增量基线：Visual Business Model Editor

本轮以 v1.1-BusinessCanvas ZIP 为唯一代码基线；保持导航、Agent、Application、数据、渠道与发布页面原设计。/business 主工作面升级为可复用编辑器，/analyst 读取当前编辑模型。

业务模型拆分为 semantic_model(nodes/edges/rules/states) 和 view_layout(node_positions/zoom/viewport/groups/mode)。规则与状态是对应类型节点的 ID 索引，文本定义存于节点 description；本轮不引入可执行规则 DSL。

语义提交生成新的 Business Model revision、语义快照、变更对象清单，清空当前模型确认、当前资产 evaluation/freeze，重置候选发布状态并进入 Impact Analysis。原型只有一个业务模型及一套审批/评测，采取保守全量失效；影响图仍使用原显式四层依赖示例，不声称实现跨模型精确依赖推导。

拖动、平移、缩放、适配、模式切换、自动整理、保存/恢复布局只改 view_layout，不增加 Business Model revision 或使审批失效。语义撤销/重做是新提交，不能恢复旧审批凭证。Business Model revision 与平台整体资产快照 revision 分别计数，其他资产编辑不增加模型 revision。

具体实现、17 项需求覆盖与生产边界见 [v1.2 编辑器交接](06-visual-model-editor.md)。以下为保留的原产品总体方案，应用编排画布的只读/不可自由连线限制仍有效。
